...
export function unidadesDeProyecto(incluirInactivas = false, proyecto = cfProyecto) {
  return state.unidades
    .filter(u => u.proyecto === proyecto && (incluirInactivas || u.activo !== false))
...
export function pagoById(id) { return state.historial.find(h => String(h.id) === String(id)); }
export function facturaById(id) { return state.facturas.find(f => String(f.factura_id) === String(id)); }
...
export function _pagosCubiertosPorFacturaSet() {
...
export function _factExistSet() { return (state.facturas && state.facturas.length) ? new Set(state.facturas.map(f => String(f.factura_id))) : null; }
export function _pagoExistSet() { return (state.historial && state.historial.length) ? new Set(state.historial.map(h => String(h.id))) : null; }
...
export function _pagosCapitalSet() {
  const set = new Set();
...
export function _tipoAsignacion(a, pagosCubiertos, factCanceladas, factExist, pagoExist, pagosCapital) {
  if (a.factura_id) {
...
function renderSubTabs() {
  ...
  // La vista 🧾 Fiscal se MUDÓ a su propia página del menú (src/modules/fiscal.js),
  // visible para admin y contabilidad. Aquí ya no hay pestaña fiscal.
...
function renderPanel() {
  ...
  // 'fiscal' ya no existe aquí: si quedó pegada en cfTab (sesión vieja), cae a Unidades.
  else renderUnidadesTab(panel);
}
...
// La antigua TAB 🧾 FISCAL (deducibilidad) vive ahora en src/modules/fiscal.js
// (página propia del menú, admin + contabilidad). Importa de aquí los helpers
// compartidos (_tipoAsignacion y los Sets) — una sola fuente de verdad.
