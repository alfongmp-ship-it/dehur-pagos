...
function resolverPartidaObra(nombreObra, proyecto) {
  const cod = normCodigo(nombreObra);
  const proy = normCodigo(proyecto);
  if (!cod) return null;
  let cand = (state.partidasObra || []).find(p =>
    p.activa !== false &&
    normCodigo(p.nombre) === cod &&
    normCodigo(p.proyecto) === proy
  );
  if (cand) return cand;
  return (state.partidasObra || []).find(p =>
    p.activa !== false &&
    normCodigo(p.nombre) === cod &&
    !p.proyecto
  ) || null;
...
export function parsearSolicitud(wb, filename) {
  ...
  wb.SheetNames.forEach(sheetName => {
    ...
    rows.slice(dataStart).forEach((r, rIdx) => {
      ...
      // Resolver partida-obra contra catálogo Obra del proyecto para mapear a Admin.
      const partidaOriginal = partida;
      let partidaResuelta = partida;
      let subPartida = '';
      let partidaObra = '';
      let mapeoEstado = 'no-catalogo'; // 'mapeada' | 'sin-mapeo' | 'no-catalogo'
      if (partidaOriginal) {
        const matchObra = resolverPartidaObra(partidaOriginal, proyectoDet);
        if (matchObra && matchObra.partidaAdmin) {
          partidaResuelta = matchObra.partidaAdmin;
          subPartida = matchObra.subPartidaAdmin || '';
          partidaObra = matchObra.nombre;
          mapeoEstado = 'mapeada';
        } else if (matchObra) {
          partidaObra = matchObra.nombre;
          mapeoEstado = 'sin-mapeo';
          warningsGlobales.push(`Fila ${numeroFila}: partida obra "${partidaOriginal}" no tiene mapeo Admin`);
        } else {
          mapeoEstado = 'no-catalogo';
          warningsGlobales.push(`Fila ${numeroFila}: "${partidaOriginal}" no existe en catálogo Obra del proyecto ${proyectoDet}`);
        }
      }
      ...
      state.solicitudesData.push({
        ...
        partida: partidaResuelta, // ya mapeada a Admin (o original si no hubo match)
        partidaOriginal,          // lo que obra escribió literal (para mostrar en UI)
        subPartida,               // sub Admin mapeada
        partidaObra,              // nombre canónico de Obra (vacío si no había en catálogo)
        mapeoEstado,
        clave, oc, concepto, motivo, importe, flag,
...
function badgePartidaObra(s) {
  const original = s.partidaOriginal || s.partida || '';
  if (!original) return '<span style="color:var(--muted);">—</span>';
  let badge = '';
...
export function renderSolicitudes() {
  ...
  const fil = state.solicitudesData.filter(s =>
    (!q || (s.proveedor.toLowerCase().includes(q) || s.concepto.toLowerCase().includes(q) || s.partida.toLowerCase().includes(q) || (s.partidaOriginal || '').toLowerCase().includes(q) || s.clave.toLowerCase().includes(q))) &&
    (!fp || s.partida === fp)
  );
...
export function enviarACola() {
  ...
  validos.forEach(s => {
    ...
    state.cola.push({
      ...
      partida: s.partida || '',
      // Sub-partida Admin (resuelta desde el mapeo Obra→Admin)
      sub_partida: s.subPartida || '',
      // Detalle obra detallado (para partida_override en costoAsignaciones)
      partidaObra: s.partidaObra || '',
      origen: 'solicitud',
